...
@Component({
  selector: 'app-following-pg',
  templateUrl: './following-pg.component.html',
  styleUrls: ['./following-pg.component.css'],
})
export class FollowingPgComponent implements OnInit {
  regUser: any = {};
  userId!: string;
  followings!: any[];
  constructor(
  ...
  ngOnInit(): void {
    this.activatedRoute.parent?.params.subscribe((params: any) => {
      ...
      this.getFollowings(params.userId);
    });
  }
  ...
  getFollowings(userId: string) {
    this.userProxyS.getFollowings(userId).subscribe((response: any) => {
